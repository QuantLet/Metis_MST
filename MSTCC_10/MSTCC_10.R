...
setwd("~/Documents/METIS/Minimum Spanning Tree/Codes/MSTCC_10")

prices<-read.table("price.csv", header=TRUE, sep=",", dec=".")
ZOO <- zoo(prices[,-1], order.by=as.Date(as.character(prices$Date), format='%m/%d/%Y'))
head(prices)

#to plot normalized prices
norm_prices<-read.table("normprice.csv", header=TRUE, sep=",", dec=".")
...
#plot(ZOO_norm[,c(3,7,8,9,10)], ylab="normalized prices",xlab="", plot.type= "single", col=c("#FF00FF","#FFFF00","#CCCCCC","#FF6600","#0000FF"))
dev.off()
...
## draw heatmap of distance matrix
png("DistMtx.png",width = 500,height = 400,bg="transparent")
mycol<-colorRampPalette(c("white","tomato"))(100)
pheatmap(ciao[[1]],cluster_rows=T,cluster_cols=T,border = F,display_numbers = T,color = mycol)
dev.off()
...
png("MSTCC_MST_firstwindow.png", width=500, height=400, bg = "transparent")
plot(mst[[1]], edge.color= "black", vertex.size= node.size, vertex.color="orange", layout=layout_with_graphopt)
dev.off()
## plot MST second window
verticesdegreeall<-degree(mst[[2]])
node.size<-as.matrix(verticesdegreeall)*6
png("MSTCC_MST_secondwindow.png", width=500, height=400, bg = "transparent")
plot(mst[[2]], edge.color= "black", vertex.size= node.size, vertex.color="orange", layout=layout_with_graphopt)
dev.off()
## plot MST last window
verticesdegreeall<-degree(mst[[t]])
node.size<-as.matrix(verticesdegreeall)*6
png("MSTCC_MST_lastwindow.png", width=500, height=400, bg = "transparent")
plot(mst[[t]], edge.color= "black", vertex.size= node.size, vertex.color="orange", layout=layout_with_graphopt)
dev.off()
## plot MST from  Sep14, 2017 to Oct 17, 2019
...
#### Portfolio Part
### Plot returns of different portfolio
